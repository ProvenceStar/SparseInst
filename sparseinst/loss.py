...
class SparseInstCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_instances, input_shape=None):
        ...
        if idx[0].shape[0] == 0:
            return {'loss_ce': (src_logits * 0).sum()}
...
class SparseInstMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, input_shape):
        with torch.no_grad():
            ...
            indices = [(torch.tensor(i, dtype=torch.int64), torch.tensor(j, dtype=torch.int64)) for i, j in indices]
            # indices = [(torch.from_numpy(i).to(torch.int64), torch.from_numpy(j).to(torch.int64)) for i, j in indices]
            return indices
